...
public class CourseSchedule_bfs_20200608 {
    ...
    private boolean bfs(Map<Integer, Set<Integer>> graph, int projects, int[][] dependencies, int[] ascii) {
    ...
    private void buildGraph(Map<Integer, Set<Integer>> graph, int numCourses, int[][] prerequisites, int[] ascii) {
        ...
        for (int j = 0; j < prerequisites.length; j++) {
            int from = prerequisites[j][1];
            int to = prerequisites[j][0];
            graph.get(from).add(to);

            if(graph.get(to).contains(from) && graph.get(from).contains(to)){
                graph.clear();
                break;
            }
